...
# Training arguments for ZeRO Infinity with NVMe offloading
training_args = TrainingArguments(
    ...
    eval_strategy="steps",
    eval_steps=100,
    deepspeed="deepspeed_infinity_config.json",  # DeepSpeed config with Infinity offloading
    report_to="wandb" if accelerator.is_main_process else None,
    run_name="bert-base-infinity-offload",
    save_strategy="steps",
    save_total_limit=2,
    dataloader_num_workers=0,  # Set to 0 to avoid multiprocessing issues
    remove_unused_columns=False,
    label_names=["labels"],  # Explicitly specify label column name
    # Additional settings for Infinity offloading
    dataloader_pin_memory=False,  # Disable pin memory when offloading to NVMe
