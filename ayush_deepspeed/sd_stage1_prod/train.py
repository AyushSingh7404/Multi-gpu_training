#!/usr/bin/env python3
"""
Production-Ready Stable Diffusion 3.5 Medium Fine-tuning Script with LoRA
=========================================================================

This script fine-tunes Stable Diffusion 3.5 Medium using LoRA (Low-Rank Adaptation) 
with DeepSpeed Stage 1 optimization for memory efficiency and faster training.

Features:
- Environment variable configuration with fallbacks
- DeepSpeed Stage 1 for distributed training
- LoRA for parameter-efficient fine-tuning
- Comprehensive logging and monitoring
- W&B integration for experiment tracking
- Robust error handling and memory management
- Production-ready checkpoint saving/loading
...
class TrainingConfig:
    ...
    save_steps: int = 500
    run_name: str = "sd35-lora-production-run"
    ...
    def __post_init__(self):
        """Initialize validation prompts if not provided."""
...
def load_environment_config() -> TrainingConfig:
    ...
    This function supports both direct environment variables and .env files,
    maintaining backward compatibility with existing Docker commands.
    ...
    config = TrainingConfig(
        ...
        # Validation settings
        num_validation_images=get_env("NUM_VALIDATION_IMAGES", TrainingConfig.num_validation_images, int),
        validation_steps=get_env("VALIDATION_STEPS", TrainingConfig.validation_steps, int),
    )
...
def setup_lora(transformer: SD3Transformer2DModel, config: TrainingConfig, logger: logging.Logger) -> SD3Transformer2DModel:
    ...
    lora_config = LoraConfig(
        ...
        bias="none",
        task_type="FEATURE_EXTRACTION",  # Appropriate task type for diffusion models
    )
...
def encode_text_batch(tokenizer: CLIPTokenizer, text_encoder: CLIPTextModel, 
                     texts: List[str], device: torch.device, logger: logging.Logger) -> Optional[torch.Tensor]:
    """
    Encode a batch of text prompts to hidden states.
    ...
    Returns:
        Encoded text hidden states or None if error
    """
    try:
        # Tokenize text
...
def encode_images_to_latents(vae: AutoencoderKL, images: torch.Tensor, 
                           device: torch.device, logger: logging.Logger) -> Optional[torch.Tensor]:
    """
    ...
    Returns:
        Encoded latents or None if error
    """
    try:
        with torch.no_grad():
        ...
        logger.error(f"Error encoding images to latents: {e}")
        return None
...
def training_step(batch: Dict, transformer: SD3Transformer2DModel, vae: AutoencoderKL, 
    ...
    """
    # Encode text
    encoder_hidden_states = encode_text_batch(
        tokenizer, text_encoder, batch["text"], accelerator.device, logger
    )
    if encoder_hidden_states is None:
        return None
    
    # Encode images to latents
    latents = encode_images_to_latents(vae, batch["images"], accelerator.device, logger)
    if latents is None:
        return None
...
def log_training_metrics(loss: float, epoch: int, step: int, global_step: int, 
...
def save_checkpoint(accelerator: Accelerator, output_dir: str, global_step: int, logger: logging.Logger) -> None:
    """
...
def main():
    ...
    try:
        ...
        logger.info("Starting Stable Diffusion 3.5 Medium fine-tuning")
        ...
        logger.info(f"Accelerator initialized with mixed_precision={config.mixed_precision}")
        ...
        if accelerator.is_main_process:
            ...
            try:
                wandb.init(
                    project="sd35-finetune-production",
        ...
        for epoch in range(config.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if global_step % config.logging_steps == 0 and accelerator.is_main_process:
                    ...
                    log_training_metrics(loss.item(), epoch, step, global_step, current_lr, logger)
                    
                    # Log additional metrics
                    if wandb.run is not None:
                        wandb.log({
                            "train/avg_loss": avg_loss,
                            "train/epoch_progress": step / len(train_dataloader),
                ...
                if global_step % config.save_steps == 0 and accelerator.is_main_process:
                    save_checkpoint(accelerator, config.output_dir, global_step, logger)
